...
- **Provenance is evidence.** For any AI-assisted result, the prompts, transcripts, check scripts, and certificate project are part of the record and are kept beside the result ("release the full process" — Gomez-Serrano). A conclusion whose derivation trail is gone has dropped a certificate tier.
...
- **Corroborate across models before trusting a fact.** For a factual claim with no mechanical certificate, ask a second model family the same question cold; disagreement flags fabrication and agreement is weak evidence, never proof (Radhakrishnan, Harvard CMSA, 2026: a 70B model told to be "extremely accurate" invents social security numbers for the famous dead). An internal or structural signal, where one exists, outranks any LLM-as-judge.
