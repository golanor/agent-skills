...
description: Build stabilizer-code simulations as plumbing on request — Stim circuits, detector error models, PyMatching/BP-OSD/Tesseract decoders, Sinter threshold sweeps, logical-error-rate plots, circuit-level distance estimates, code-family scans, and formal circuit certification (flow-based gadget proofs, rowspace reconstruction, p=0 determinism, hash-pinned certificates) — from the user's stated spec, inside notebook-pair's `# @agent:` marker protocol. Use when the user asks for a stim circuit, a memory experiment, a DEM, a decoder hookup, a threshold or pseudo-threshold sweep, a circuit-level distance, a code-capacity or circuit-level noise model, a gadget or circuit certificate, or a stabilizer/logical-operator table for a CSS, GB, BB, APM, or surface code.
version: 1.2.0
...
| `stimbposd` | BP-OSD directly on a stim DEM — the default BP-OSD path |
| `tesseract-decoder` | Most-likely-error search decoder (Google); qLDPC circuit noise, circuit-distance probes |
...
A buildable spec names: **code** (family + parameters, or explicit `H_X`, `H_Z`), **experiment** (memory-X/Z, stability, a logical gate), **rounds** `d` or `n_rounds`, **noise model** (code-capacity `p`, phenomenological `p_data, p_meas`, or circuit-level `p` with the standard gate set: depolarize after 1q/2q gates, flip before measurement, flip after reset), **decoder** (MWPM / BP-OSD with `osd_order`, `max_iter` / Tesseract), and **sweep** (`p` list, shots or `max_errors`). A marker missing any of these gets one `[needs]` question naming the missing field — never a guessed default silently applied.
...
**Noise model as an explicit object.** Keep noise in one named class (`CircuitNoise`-style) with one constructor per model the user names, so the DEM's provenance is a single symbol: e.g. `uniform(p)` = `DEPOLARIZE2` after each `CX`, flip before measure, flip after reset, one idle `DEPOLARIZE1` per data qubit per round; a pair-measurement model = one combined channel per `MZZ`/`MXX` written out as its `CORRELATED_ERROR` mechanisms plus prep/measure flips on the free single-qubit ops plus the same once-per-round idle — name it **EM3** in the docstring when it matches Gidney's SD6/SI1000/EM3 vocabulary, so results compare against the literature. Apply noise by transforming the certified noise-free circuit, never by hand-editing it. Anything an "every-gate-p" shorthand hides (which ops idle, whether reset flips count) is stated once in the noise class's docstring.

**Detector error model and decoder.** `dem = circuit.detector_error_model(decompose_errors=True)` for MWPM via `pymatching.Matching.from_detector_error_model(dem)`. BP-OSD takes the **non-decomposed** DEM through `stimbposd.BPOSD(dem, max_bp_iters=..., osd_order=...)`; hand-roll the check matrix + priors via `ldpc` only when `stimbposd` is unavailable. Tesseract (`tesseract_decoder.tesseract`, config: `pqlimit`, `det_beam`) is the third option; its `low_confidence_flag` means *no error set was found* — raise, never return the prediction buffer silently. A circuit with combined pair-measurement channels needs `approximate_disjoint_errors=True` at DEM extraction, or stim refuses the composite channel. For non-matchable codes `decompose_errors=True` will fail — report it and switch to BP-OSD without being asked to explain why.
...
**Circuit-level distance** (adapted from shohamjac/stairway-codes, the "Stairway Codes" companion repo — pattern, not code). Replace one `OBSERVABLE_INCLUDE` at a time with a `DETECTOR` tagged `Logical`, build the DEM, and hand the decoder a syndrome in which only that logical detector fires: the predicted error set is a fault path flipping the logical undetected, so its weight is an **upper bound** on circuit distance. Minimize over logicals and report which logical achieved it. Rung labels: a BP-OSD or Tesseract weight is *estimated* (a heuristic decoder can overshoot the true minimum); an ILP over the DEM (Gurobi-class, time-limited) is *proved* on convergence within its limit, otherwise reported as bound + timeout.

**Code-family scan.** When the sweep axis is candidate codes rather than `p`: materialize candidates as a CSV of construction parameters, filter with a pandas query string, and run per-row build → certify → (circuit-)distance with an explicit per-row time limit. Cache per-row results keyed by config hash exactly as the LER sweep does, so a widened scan reuses finished rows and shards split across machines by row range.
